...
@app.route('/')
def index():
    return "I'm alive" # Trả về dòng này khi truy cập web

def run_http():
    # Render yêu cầu chạy trên port 0.0.0.0
    app.run(host='0.0.0.0', port=8080)

# --- Phần code bot/tool của bạn ---
def run_bot():
    print("Bot đang chạy...")
    while True:
        # Code xử lý chính của bạn ở đây
        print("Đang làm việc...")
        time.sleep(10) # Ví dụ nghỉ 10s
...
if __name__ == '__main__':
    keep_alive()  # Chạy web server ở luồng phụ
    run_bot()     # Chạy bot ở luồng chính
